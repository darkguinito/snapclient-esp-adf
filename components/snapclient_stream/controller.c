...
// 	char *   message_name = get_message_type(type);
// 	audio_element_info_t snap_info = {0};
// 	ESP_LOGI(TAG, "%s", message_name);
// 	switch (type) {
// 		case SNAPCAST_MESSAGE_CODEC_HEADER:
// 		{
...
// 			for(int i = 0; i < codec_header_message->size ; i++){
// 				ESP_LOGI(TAG, "i: %d, char: %c %X", i, codec_header_message->payload[i], codec_header_message->payload[i]);
// 			}
...
// 			// buffer_ms          = server_settings_message.buffer_ms - snapclient->server_settings_message.latency - snapclient->player.latency;
...
// 		default:
// 		{
// 			ESP_LOGW(TAG, "UNKNOWN_MESSAGE_TYPE %d ", type);
// 			break;
// 		}
// 	}
// 	return;
